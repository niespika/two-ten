...
export function ChronoScreen({ state, onDone }: { state: ChronoState; onDone: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex flex-col items-center justify-center bg-brand-bg px-4 dark:bg-brand-darkBg">
      <OrnateFrame minimal className="w-full max-w-[420px] text-center">
        <p className="mb-6 text-sm uppercase tracking-[0.36em] opacity-65">{countdownLeft > 0 ? 'Get Ready' : 'Effort'}</p>
        <div className="font-serif text-8xl text-[#173a35] transition-all duration-500 ease-out dark:text-brand-darkText">{text}</div>
